...
from dataset import VITONDataset, VITONDataLoader
...
def test(opt, seg, gmm, alias):
    ...
    print("TEST_DATASET",vars(test_dataset))
    test_loader = VITONDataLoader(opt, test_dataset)
    print("TEST_LOADER",vars(test_loader))
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            print("PARSE AGNOSTIC",parse_agnostic.size())
            parse_agnostic_down = F.interpolate(parse_agnostic, size=(256, 192), mode='bilinear')
            pose_down = F.interpolate(pose, size=(256, 192), mode='bilinear')
            c_masked_down = F.interpolate(c * cm, size=(256, 192), mode='bilinear')
            cm_down = F.interpolate(cm, size=(256, 192), mode='bilinear')
            print("cm_down shape:", cm_down.shape)
            print("c_masked_down shape:", c_masked_down.shape)
            print("parse_agnostic_down shape:", parse_agnostic_down.shape)
            print("pose_down shape:", pose_down.shape)
            print("gen_noise(cm_down.size()) shape:", gen_noise(cm_down.size()).shape)
